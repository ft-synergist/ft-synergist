...
export default function VietnamAuditForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Completely standalone client-side generation path ensuring zero network failures
    setTimeout(() => {
      setReport({
    ...
    <div className="w-full max-w-4xl mx-auto bg-neutral-950 p-8 md:p-12 border border-neutral-800 rounded-none text-white my-8 text-left font-sans">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-xs font-bold text-neutral-400 uppercase tracking-widest mb-2">Core Industry Sector</label>
            <input
              type="text"
              required
              className="w-full px-4 py-3 bg-neutral-900 border border-neutral-800 text-white focus:outline-none focus:border-[#8F801B] focus:ring-1 focus:ring-[#8F801B] transition-all rounded-none placeholder-neutral-600 text-sm"
          ...
          <div>
            <label className="block text-xs font-bold text-neutral-400 uppercase tracking-widest mb-2">Primary Strategic Focus Area</label>
            <select
              className="w-full px-4 py-3 bg-neutral-900 border border-neutral-800 text-white focus:outline-none focus:border-[#8F801B] focus:ring-1 focus:ring-[#8F801B] transition-all rounded-none text-sm cursor-pointer"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full inline-flex items-center justify-center px-8 py-4 bg-[#8F801B] text-white font-bold uppercase tracking-wider text-xs hover:bg-[#A08d1e] transition-all shadow-lg rounded-none disabled:opacity-70"
        >
          {loading ? "Evaluating Market Parameters..." : "Generate Expansion Diagnostics Report"}
        </button>
      </form>
        ...
        <div className="mt-12 border border-neutral-800 bg-neutral-900 p-8 rounded-none space-y-6 animate-in fade-in duration-300">
          ...
          <div className="space-y-6 text-neutral-400 text-sm leading-relaxed font-sans">
            ...
            <div className="border-t border-neutral-800 pt-4">
              ...
              <p>Operations must absorb adjusted insurance computation baselines under Decree 161/2026/ND-CP. Mandatory SHUI (Social, Health, and Unemployment Insurance) contribution indices account for 34% of overall gross individual compensation streams. Employment documentation requires processing unique 13-character tracking IDs via state API integrations under Circular 08/2026/TT-BNV.</p>
            </div>

            <div className="border-t border-neutral-800 pt-4">
              <h5 className="text-white font-bold text-xs uppercase tracking-wider mb-2">4. Supply Chain Allocation Moat &amp; Subsidies</h5>
              <p>By mapping your operational strategy into regional processing clusters, you bypass the Legacy Export Trap. Your infrastructure setup qualifies for up to 70% grant co-funding support capped at S$100,000 per target market under Enterprise Singapore's updated internationalization pipeline guidelines.</p>
            </div>
          </div>
        </div>
      )}
